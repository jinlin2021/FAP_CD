...
# hyperparams keep the same for DPM-Solver-1, DPM-Solver-2 and DPM-Solver-3 

import torch
import numpy as np
import functools
from datasets import get_nodes
...
def dpm_mol_solver_3(atom_sde, bond_sde, theta_fn, x_atom_last, x_bond_last,condition_features,
    ...
    atom_theta_0, bond_theta_0 = theta_fn((x_atom_last, x_bond_last), vec_t_last,
                                          atom_mask=atom_mask, bond_mask=bond_mask, urban_attr = condition_features)
    ...
    atom_theta_s1, bond_theta_s1 = theta_fn((u_atom_1, u_bond_1), s1, atom_mask=atom_mask, bond_mask=bond_mask, urban_attr = condition_features)
    ...
    atom_theta_s2, bond_theta_s2 = theta_fn((u_atom_2, u_bond_2), s2, atom_mask=atom_mask, bond_mask=bond_mask, urban_attr = condition_features)
...
def sampler_dpm3(atom_sde, bond_sde, atom_shape, bond_shape, inverse_scaler,
                         time_step, eps=1e-3, denoise=False, device='cuda'):
    # arrange time schedule
    ...
    def sampler(model, condition_model, batch, node_list):
        ...
        with torch.no_grad():
            ...
            condition_features = condition_model(demand, urban_attr, pp, node_mask).to(device)

    
            atom_mask = torch.zeros((atom_shape[0], atom_shape[1]), device=device)
            grid_ids = grid_ids.cpu().numpy()
            n_nodes = get_nodes(grid_ids, node_list)
            ...
            t_last = time_steps[0]
            for t_i in time_steps[1:]:
                x_atom, x_bond = dpm_mol_solver_3(atom_sde, bond_sde, theta_fn, x_atom, x_bond, condition_features, t_last, t_i,
                                                  atom_mask, bond_mask)
                t_last = t_i
            ...
            x_atom = inverse_scaler(x_atom, atom=True) * atom_mask.unsqueeze(-1)
            x_bond = inverse_scaler(x_bond, atom=False) * bond_mask
            return x_atom, x_bond, num_step * 3, n_nodes, atom_mask, bond_mask
