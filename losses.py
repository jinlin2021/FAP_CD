...
def get_multi_sde_loss_fn(atom_sde, bond_sde, train, reduce_mean=True, continuous=True, eps=1e-5):
    ...
    def loss_fn(model, condition_model, batch):
        ...
        atom_feat, atom_mask, bond_feat, bond_mask, house_node_mask, urban_attr,\
            demand, pp, grid_ids = batch
        # print("bond_feat:", bond_feat)
        condition_attr = condition_model(demand, urban_attr,pp, atom_mask)  #[B,M,14]
        ...
        # perturbing atom
        z_atom = torch.randn_like(atom_feat)  # [B, N, C]  生成z_atom 张量和 atom_feat 有相同的形状和数据类型，但是其内容是从标准正态分布（均值为 0，标准差为 1）中随机生成的。
        ...
        #kwargs[''],在模型cdgs中
        atom_score, bond_score = score_fn((perturbed_atom, perturbed_bond), t, atom_mask=atom_mask, bond_mask=bond_mask, urban_attr = condition_attr)

        # atom loss
        atom_mask = atom_mask[:, :, None].repeat(1, 1, atom_feat.shape[-1])
        atom_mask = atom_mask.reshape(atom_mask.shape[0], -1)
        # 计算 生成样本atom_score与原始分布 z_atom之间差异的平方损失
        ...
        if reduce_mean:
            ...
            losses_atom = 0.5 * torch.sum(losses_atom * atom_mask, dim=-1)  #torch.Size([4])
        ...
        # 计算 生成样本bond_score与原始分布 z_atom之间差异的平方损失
        losses_bond = torch.square(bond_score * std_bond[:, None, None, None] + z_bond)
        losses_bond = losses_bond.reshape(losses_bond.shape[0], -1)  #  torch.Size([4, 160000])
        if reduce_mean:
...
def get_step_fn(sde, train, optimize_fn=None, reduce_mean=False, continuous=True, likelihood_weighting=False):
    ...
    def step_fn(state, condition, batch):
        ...
        if train:
            ...
            optimizer.zero_grad()
            loss = loss_fn(model, condition, batch)
            loss.backward()
...
def get_step_loss(sde, train, optimize_fn=None, reduce_mean=False, continuous=True, likelihood_weighting=False):
